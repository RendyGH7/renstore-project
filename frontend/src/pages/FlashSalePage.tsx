          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredItems.map((item) => {
              ...
              return (
                <div
                  key={product.id}
                  className={`bg-white rounded-3xl border border-slate-200/90 shadow-xs hover:shadow-xl hover:shadow-rose-500/10 hover:-translate-y-1.5 transition-all duration-200 ease-out overflow-hidden flex flex-col justify-between group relative ${
                ...
                >
                  ...
                  <div className="absolute top-3.5 left-3.5 z-20 flex flex-col gap-1">
                    <span className={`px-2.5 py-1 rounded-xl text-white font-black text-[11px] shadow-md flex items-center gap-1 ${
                      isSlotLive ? 'bg-gradient-to-r from-red-600 to-rose-600 shadow-red-600/30' : 'bg-slate-700 shadow-slate-900/20'
                    }`}>
                      <Flame className="w-3 h-3 text-amber-300" />
                      -{discountPercent}%
                    </span>
                    <span className={`px-2 py-0.5 rounded-lg text-white font-black text-[9px] uppercase tracking-wider shadow-xs ${
                  ...
                  <div>
                    ...
                    <div className="p-5 space-y-3">
                      <div className="space-y-1">
                        ...
                        <Link
                          to={`/products/${product.slug}`}
                          className="block text-sm font-bold text-slate-900 group-hover:text-rose-600 transition-colors duration-150 line-clamp-2 leading-snug"
                        >
                          {product.name}
                        </Link>
                      </div>
                      ...
                      <div className="space-y-1.5 pt-1">
                        <div className="flex items-center justify-between text-[11px] font-bold">
                          <span className={`${isAlmostGone && isSlotLive ? 'text-rose-600' : 'text-slate-600'}`}>
                            {isAlmostGone && isSlotLive ? (
                              <span className="flex items-center gap-1 font-black text-rose-600 animate-pulse">
                                <Zap className="w-3 h-3 text-rose-600" />
                                {language === 'en' ? `Only ${stockTotal - soldCount} left!` : `Sisa ${stockTotal - soldCount} unit!`}
                              </span>
                            ) : (
                  ...
                  <div className="p-5 pt-0 flex items-center gap-2">
                    {isSlotLive ? (
                      ...
                      <button
                        type="button"
                        disabled
                        className="w-full py-2.5 px-4 rounded-full font-bold text-xs flex items-center justify-center gap-2 bg-amber-50 text-amber-700 border border-amber-200 cursor-not-allowed select-none"
                      >
                        <Clock className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                        <span className="truncate">
                          {language === 'en' ? `Upcoming (${selectedSlotData.time.split(' - ')[0]} WIB)` : `Mulai Jam ${selectedSlotData.time.split(' - ')[0]} WIB`}
                        </span>
                      </button>
                    ) : (
                      <button
                        type="button"
                        disabled
                        className="w-full py-2.5 px-4 rounded-full font-bold text-xs flex items-center justify-center gap-2 bg-slate-100 text-slate-400 border border-slate-200 cursor-not-allowed select-none"
                      >
                        <Lock className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        <span className="truncate">
                          {language === 'en' ? 'Session Ended' : 'Sesi Telah Berakhir'}
                        </span>
                      </button>
                    )}
                  </div>
